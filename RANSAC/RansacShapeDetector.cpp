...
size_t RansacShapeDetector::StatBucket(float score) const
{
  return (size_t)std::max(
      0.f,
      std::floor(
          (std::log(score) - std::log((float)m_options.m_minSupport)) /
          std::log(1.21f)) + 1);
}
...
void RansacShapeDetector::UpdateLevelWeights(
    float factor,
    ...
    std::vector< double > *sampleLevelProbability) const
{
  ...
  for(size_t i = 0; i < newSampleLevelProbability.size(); ++i)
  {
    if((*sampleLevelProbability)[i] > 0)
      newSampleLevelProbability[i] =
          (levelScores[i].first / (*sampleLevelProbability)[i]);
    else
  ...
  {
    newSampleLevelProbability[i] =
        .9f * newSampleLevelProbability[i] +
        .1f * newSampleLevelProbabilitySum/levelScores.size();
    newSum += newSampleLevelProbability[i];
  }
  
  for(size_t i = 0; i < sampleLevelProbability->size(); ++i)
  {
    (*sampleLevelProbability)[i] =
        (1.f - factor) * (*sampleLevelProbability)[i] +
    ...
    CandidatesType *candidates) const
{
  ...
  //#pragma omp parallel
  {
    ...
    for(int candIter = 0; candIter < 200; ++candIter)
    {
      ...
      if(!DrawSamplesStratified(
             globalOctree, m_reqSamples, sampleLevel,
              ...
              iend = m_constructors.end(); i != iend; ++i)
      {
        ...
        {
          shape->DistanceAndNormalDeviation(
              samplePoints[i], samplePoints[i + c], &dn);
...
void
SelectionSort(
    std::vector<Candidate>::iterator first,
    std::vector<Candidate>::iterator last)
{
  std::vector<Candidate>::iterator i = first;
  for ( ; i != last; ++i)
  {
    std::vector<Candidate>::iterator m = i;

    for (std::vector<Candidate>::iterator j = i + 1; j != last; ++j)
    {
      if ((*j) < (*m))
      {
        m = j;
...
void
SelectionSort(InputIterator first, InputIterator last, Cmp cmp)
{
  ...
  for ( ; i!=last; ++i)
  {
    ...
    for (InputIterator j = i + 1; j != last; ++j)
    {
      if (cmp(*j, *m))
      {
        m = j;
    ...
    // swap the items corresponding to iterators i and m
    typename std::iterator_traits<InputIterator>::value_type temp;
    *m = *i;
    *i = temp;
  }
}
...
template< class ScoreVisitorT >
bool
RansacShapeDetector::FindBestCandidate(
    ...
    float *maxForgottenCandidate, float *candidateFailProb) const
{
  ...
  // DEBUG
  // Usually the error happens during the sort.
  // Replace the stl sort by a hand made sort to see more easily
  // where the problem occurs.
  SelectionSort(candidates.begin(), candidates.end());
  // --> at first I thought that the problem was gone by using this sort
  // but the problem happens as well
  ...
  if(!candHeap.size())
  {
    std::cout << "not candHeap.size()" << candHeap.size() << std::endl;
    return false;
  }
  ...
  while(candHeap.size())
  {
    if(trial->IsEquivalent(
           *candHeap.front(), pc, m_options.m_epsilon,
    ...
    {
      // See debug.txt.
      // Commenting the two following lines prevent an infinite loop to occur.
  ...
  while(bestCandidateFailureProbability <= m_options.m_probability
        && trial->UpperBound() >= minSize
        && trial->ImproveBounds(
            octrees, pc, scoreVisitor, currentSize,
            m_options.m_bitmapEpsilon, octrees.size()))
  {
    bestCandidateFailureProbability = CandidateFailureProbability(
        trial->ExpectedValue(),
  ...
  for(size_t i = bestCandidate - 1; i != -1; --i)
  {
    ...
    if(iFailProb > m_options.m_probability
    ...
    if(candidates[bestCandidate].IsEquivalent(
           candidates[i], pc,
           m_options.m_epsilon, m_options.m_normalThresh))
    {
      continue;
    ...
    do
    {
      ...
      {
        bool dontBreak =
            candidates[i].ImproveBounds(
                octrees, pc, scoreVisitor,
                currentSize, m_options.m_bitmapEpsilon,
                maxImproveSubsetDuringMaxSearch);
        iFailProb = CandidateFailureProbability(
            candidates[i].ExpectedValue(),
            currentSize - numInvalid, drawnCandidates, numLevels);
      ...
      else if(candidates[bestCandidate].UpperBound() >
              candidates[i].UpperBound()
              && candidates[bestCandidate].LowerBound() <
              candidates[i].LowerBound())
      {
        bool dontBreak =
            candidates[bestCandidate].ImproveBounds(
                octrees, pc,
    ...
    if((candidates[i] > candidates[bestCandidate]
       ...
       && (iFailProb <= m_options.m_probability
           && candidates[i].UpperBound() >= minSize))
    {
      while(iFailProb <= m_options.m_probability
            && candidates[i].UpperBound() >= minSize
  ...
  while(candidates[bestCandidate].ImproveBounds(
            octrees, pc, scoreVisitor,
            currentSize, m_options.m_bitmapEpsilon, octrees.size()));
  ...
  if((bestCandidateFailureProbability <= m_options.m_probability
      && candidates[bestCandidate].UpperBound() >= minSize)
     || (m_autoAcceptSize
         && candidates[bestCandidate].UpperBound() >= m_autoAcceptSize))
  {
...
RansacShapeDetector::Detect(
    PointCloud &pc, size_t beginIdx, size_t endIdx,
    std::vector< std::pair< RefCountPtr< PrimitiveShape >, size_t > > *shapes)
{
  ...
  ScorePrimitiveShapeVisitor< FlatNormalThreshPointCompatibilityFunc,
      ImmediateOctreeType > subsetScoreVisitor(
          m_options.m_epsilon,
          m_options.m_normalThresh);
  ScorePrimitiveShapeVisitor< FlatNormalThreshPointCompatibilityFunc,
      IndexedOctreeType > globalScoreVisitor(
          3 * m_options.m_epsilon,
          m_options.m_normalThresh);
  ...
  do {
    ...
    bestExpectedValue =
        std::min((float)(currentSize - numInvalid), bestExpectedValue);
    ...
    while(FindBestCandidate(
              candidates, octrees, pc, subsetScoreVisitor,
              ...
              &bestCandidateFailureProbability))
    {
      ...
      if(!foundCandidate)
      {
        ...
        {
          if(candidates[i].ExpectedValue() * 1.4f >
             candidates.back().ExpectedValue())
          {
            size_t candLevel =
                std::min(sampleLevelScores.size() - 1,
                         candidates[i].Level());
      ...
      if(m_options.m_fitting != Options::NO_FITTING)
      {
        ...
        // get the weight once
        newScore = clone.GlobalWeightedScore(
            globalScoreVisitor, globalOctree,
        ...
        do {
          ...
          PrimitiveShape *shape;
          if(shape = Fit(
                 allowDifferentShapes, *clone.Shape(),
      ...
      // remove the points
      shapes->push_back(
          std::make_pair(RefCountPtr< PrimitiveShape >(candidates.back().Shape()),
      ...
      drawnCandidates = std::pow(
          1.f - (candidates.back().Indices()->size() /
      ...
      if(numInvalid > currentSize / 4) // more than half of the points assigned?
      {
        ...
        for(intptr_t i = 0; i < candidates.size(); ++i)
          candidates[i].Reindex(
              shapeIndex, minInvalidIndex, mergedSubsets,
        ...
        globalOctree.IndexedRange(
            globalOctreeIndices.begin(),
      ...
      /*
        std::sort(
          candidates.begin(), candidates.end(), std::greater< Candidate >());
      */
      SelectionSort(
          candidates.begin(), candidates.end(), std::greater< Candidate >());
    ...
    else
    {
      ...
      m_options.m_minSupport, currentSize - numInvalid,
  ...
  for(size_t i = 0; i < shapes->size(); ++i)
  {
    ...
    (*shapes)[i].first->OptimizeParametrization(
        pc, bidx, eidx,
        m_options.m_bitmapEpsilon);
    eidx = bidx;
  }
...
bool
RansacShapeDetector::DrawSamplesStratified(
    ...
    const IndexedOctreeType::CellType **node) const
{
  for(size_t tries = 0; tries < m_maxCandTries; tries++)
  {
    ...
    *node = oct.NodeContainingPoint(
        oct.at(first), depth, numSamples,
        &nodeRange);
      ...
      do
      {
        i = oct.Dereference(
            ...
            + nodeRange.first);
                 ...
                 ||
                 std::find(
                     samples->begin(), samples->end(), i) != samples->end() )
             && iter++ < 40);
...
PrimitiveShape *RansacShapeDetector::Fit(
    ...
    std::pair< size_t, float > *score) const
{
  ...
  if(m_options.m_fitting == Options::LS_FITTING)
    bestShape = initialShape.LSFit(
        pc, m_options.m_epsilon,
